function t_menu__highlightActiveLinks(selector) {
    ...
    Array.prototype.forEach.call(shouldBeActiveElements, (function(element) {
        ...
        if (slicedPath) {
            ...
            fullPath !== url && fullPath !== path && slicedPath !== url && slicedPath !== path && urlWithoutSlash !== url && urlWithoutSlash !== path || element.classList.add("t-active")
        }
    }))
}
function t_menu__findAnchorLinks(recid, selector) {
    if (!(window.innerWidth < 980)) {
        var rec = document.getElementById("rec" + recid),
            anchorSelector = selector + '[href*="#"]:not(.tooltipstered)',
...
function t_menu__updateSectionsOffsets(sections) {
    sections.forEach((function(section) {
        ...
        section.setAttribute("data-offset-top", sectionTopPos.toString())
    }))
}
function t_menu__getSectionByHref(curlink) {
    if (curlink) {
        ...
        if (0 === curLinkValue.indexOf("/") && (curLinkValue = curLinkValue.slice(1)), href && curlink.matches('[href*="#rec"]'))
            return curLinkValue = curLinkValue.replace(/.*#/, ""), document.getElementById(curLinkValue);
...
function t_menu__highlightNavLinks(navLinks, sections, sectionToNavigationLinkID, clickedSectionID) {
    ...
    if (sections.length && null === clickedSectionID && lastSectionTopPos > scrollPosition + 300)
        return navLinks.forEach((function(link) {
...
function t_menu__showFixedMenu(recid, selector) {
    ...
    if (!(window.innerWidth <= 980) || isHasExceptionClass) {
        ...
        Array.prototype.forEach.call(menuBlocks, (function(menu) {
            ...
            if (appearOffset) {
                -1 !== appearOffset.indexOf("vh") && (appearOffset = Math.floor(window.innerHeight * (parseInt(appearOffset) / 100))),
...
function t_menu__changeBgOpacity(recid, selector) {
    var exceptSelectorList,
        isExceptSelector = ["t280", "t282", "t451", "t466"].some((function(expectClass) {
    ...
    if (!(window.innerWidth <= 980) || isExceptSelector) {
        ...
        Array.prototype.forEach.call(menuBlocks, (function(menu) {
            ...
            window.pageYOffset > 20 && "0" === bgOpacityTwo || window.pageYOffset <= 20 && "0.0" === bgOpacity || " " === menuShadow ? menu.style.boxShadow = "none" : menu.style.boxShadow = "0px 1px 3px rgba(0,0,0," + menuShadowValue + ")"
        }))
    }
}
function t_menu__createMobileMenu(recid, selector) {
    ...
    if (rec) {
        ...
        if (mobileMenu)
            if (menu && menu.classList.contains(selector.slice(1) + "__mobile_burgerhook") && menuMobileBurgerhook) {
                ...
                if (mobileBurger) {
                    ...
                    mobileBurgerParent && (link.appendChild(mobileBurger), mobileBurgerParent.appendChild(link))
                }
            } else {
                var mobileBurger = mobileMenu.querySelector(".t-menuburger");
                mobileMenu.addEventListener("click", (function(event) {
                    if (!event.target.closest("a")) {
                        ...
                        else if (t_menu__fadeIn(menu, 300, (function() {
                            menu.style.transform && (menu.style.transform = ""),
                            menu.style.opacity && (menu.style.opacity = "")
                        })), mobileMenu.classList.add(mobileMenuOpenClass), mobileBurger.classList.add(burgerOpenClass), menu.classList.contains("tmenu-mobile__menucontent_fixed")) {
        ...
        window.innerWidth < 980 && rec.addEventListener("click", (function(e) {
            if (menu && menu.classList.contains("tmenu-mobile__menucontent_fixed")) {
                var menuLink = e.target.closest(".t-menu__link-item, .t978__submenu-link, .t966__menu-link, .t-menusub__link-item, .t-btn"),
                    submenuClassList,
                    isSubmenuHook;
                if (menuLink)
                    ["t978__menu-link_hook", "t978__tm-link", "t966__tm-link", "t794__tm-link", "t-menusub__target-link"].some((function(submenuClass) {
        ...
        window.addEventListener("resize", t_throttle((function() {
            window.innerWidth > 980 && (menu && (menu.style.opacity = ""), menu && (menu.style.display = ""), menu && (menu.style.top = ""), mobileMenu && mobileMenu.classList.remove(mobileMenuOpenClass)),
            t_menu_checkOverflow(recid, selector)
        }), 200))
    }
}
...
function t_menu__FadeOut(element, duration, callback) {
    ...
    var speed,
        timer = setInterval((function() {
            ...
            (opacity -= .1) <= .1 && (element.style.opacity = "0", element.style.display = "none", "function" == typeof callback && callback(), clearInterval(timer))
        }), duration > 0 ? duration / 10 : 40)
}
function t_menu__fadeIn(element, duration, callback) {
    if (!element)
        return !1;
    if (("1" === getComputedStyle(element).opacity || "" === getComputedStyle(element).opacity) && "none" !== getComputedStyle(element).display)
        return !1;
    var opacity = 0,
    ...
    var timer = setInterval((function() {
        element.style.opacity = opacity,
        (opacity += .1) >= 1 && (element.style.opacity = "1", "function" == typeof callback && callback(), clearInterval(timer))
    }), speed)
}
function t_menu__slideElement(menu, menuHeight, direction) {
    var diff = "toTop" === direction ? 0 : menuHeight,
        ...
        timerID = setInterval((function() {
            ...
            diffOpacity = "toTop" === direction ? diffOpacity - .1 : diffOpacity + .1,
            diff = "toTop" === direction ? diff + menuHeight / 20 : diff - menuHeight / 20,
            "toTop" === direction && diff >= menuHeight && (menu.style.transform = "translateY(-" + menuHeight + "px)", menu.style.opacity = "0", clearInterval(timerID)),
            "toBottom" === direction && diff <= 0 && (menu.style.transform = "translateY(0px)", menu.style.opacity = "1", clearInterval(timerID))
        }), 10)
}
