...
window.tildaForm.addMebersInfoToForm = function(form) {
    ...
    try {
        ...
        if (mauserInfo && t_removeEl(mauserInfo), !window.mauser || !window.mauser.code || !window.mauser.email)
            return !1;
...
window.tildaForm.successEnd = function(form, successUrl, successCallback) {
    ...
    successBox && (successBox.textContent && successBox.innerText || !successStr || (successBox.innerHTML = successStr), "y" === form.getAttribute("data-success-popup") ? window.tildaForm.showSuccessPopup(successBox.innerHTML) : successBox.style.display = "block"),
...
window.tildaForm.send = function(form, btnSubmit, formType, formKey) {
    ...
    if (inputItsGood || form.insertAdjacentHTML("beforeend", '<div style="position: absolute; left: -5000px; bottom: 0; display: none;"><input type="text" name="form-spec-comments" value="Its good" class="js-form-spec-comments" tabindex="-1" /></div>'), 2 === formType || !formType && formKey) {
        ...
        })), dataForm = arrFilter, window.tildaForm.tildapayment && window.tildaForm.tildapayment.products)
            dataForm.push({
                name: "tildapayment",
                        ...
                        if ("object" == typeof objData) {
                            ...
                            if (t_removeClass(btnSubmit, "t-btn_sending"), btnSubmit.tildaSendingStatus = "0", objData && objData.error) {
                                ...
                                if (objData && objData.next && objData.next.type)
                                    return window.tildaForm.payment(form, objData.next), dataSuccessUrl = "", !1;
        ...
        return xhr.open("POST", form.getAttribute("action"), !0), xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8"), xhr.setRequestHeader("Accept", "text/plain, */*; q=0.01"), xhr.onreadystatechange = function() {
            if (4 === xhr.readyState)
                if (xhr.status >= 200 && xhr.status < 400) {
                    ...
                    Array.prototype.forEach.call(allError, (function(error) {
                        data ? error.innerHTML = data + ". Please, try again later. [" + xhr.status + "]" : xhr.statusText ? error.innerHTML = "Error [" + xhr.statusText + "]. Please, try again later. [" + xhr.status + "]" : error.innerHTML = "Unknown error. Please, try again later. [" + xhr.status + "]",
                        error.style.display = "block"
                    })),
                    Array.prototype.forEach.call(errorBoxes, (function(error) {
                        error.style.display = "block"
...
try {
    ...
    if (-1 !== TILDAPAGE_URL.toLowerCase().indexOf("utm_") && (TILDAPAGE_URL = TILDAPAGE_URL.toLowerCase(), TILDAPAGE_QUERY = TILDAPAGE_URL.split("?"), TILDAPAGE_QUERY = TILDAPAGE_QUERY[1], "string" == typeof TILDAPAGE_QUERY)) {
        ...
        for (i in arParams)
            "function" != typeof arParams[i] && (arPair = arParams[i].split("="), "utm_" == arPair[0].substring(0, 4) && (TILDAPAGE_UTM = TILDAPAGE_UTM + arParams[i] + "|||"));
...
function t_forms__initBtnDblClick(event) {
    var target;
    if (((event = event || window.event).target || event.srcElement).closest('[type="submit"]'))
...
function checkVerifyTildaCaptcha(event) {
    if (-1 !== (event = event || window.event).origin.indexOf(window.tildaForm.endpoint)) {
        ...
        if ("closeiframe" == event.data)
            return formCaptchaBox && t_removeEl(formCaptchaBox), void (tildaCaptcha && t_removeEl(tildaCaptcha));
...
function t_forms__onSuccess(form) {
    ...
    if (successBoxOffset > window.scrollY || documentHeight - inputBottom < windowHeight - 100)
        ...
        windowHeight > documentHeight && tildaLabel && setTimeout((function() {
...
function t_forms__scrollBeginForm(target) {
    ...
    function t_forms__animateScroll() {
        ...
        currentTime < 400 ? setTimeout(t_forms__animateScroll, 16) : document.body.removeAttribute("data-scrollable")
    }
...
function t_serializeArray(form) {
    for (var arr = [], elements = form.querySelectorAll("input, textarea, button, select"), i = 0; i < elements.length; i++)
        if (!(!elements[i].name || elements[i].disabled || ["file", "reset", "submit", "button"].indexOf(elements[i].type) > -1))
            if ("select-multiple" !== elements[i].type)
                ["checkbox", "radio"].indexOf(elements[i].type) > -1 && !elements[i].checked || arr.push({
                    name: elements[i].name,
                    value: elements[i].value
                });
            else
                for (var options = elements[i].options, j = 0; j < options.length; j++)
                    options[j].selected && arr.push({
                        name: options[j].name,
    ...
    if ("none" !== el.style.display)
        var opacity = 1,
            timer = setInterval((function() {
                ...
                (opacity -= .1) <= .1 && (clearInterval(timer), el.style.display = "none", el.style.opacity = null)
            }), 30)
}
